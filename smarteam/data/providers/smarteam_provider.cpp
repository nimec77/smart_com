...
SmarteamProvider SmarteamProvider::SmarteamCreate() {
  ...
  auto clsid = data_helper::get_classId(kSmarTeamProdId);
  IDispatch *app{};
  auto hr = CoCreateInstance(clsid, nullptr, CLSCTX_LOCAL_SERVER, IID_IDispatch, (void **) &app);
  if (FAILED(hr)) {
    data_helper::failed_exception("SmarteamProvider::SmarteamCreate CoCreateInstance error:", hr);
  }
...
SmarteamProvider SmarteamProvider::SmarteamFromActiveObject() {
  ...
  if (FAILED(hr)) {
    data_helper::failed_exception("SmarteamProvider::SmarteamFromActiveObject GetActiveObject error:", hr);
  }
  IDispatch *app{};
  hr = i_unknown->QueryInterface(IID_IDispatch, (void **) &app);
  i_unknown->Release();
  if (FAILED(hr)) {
    data_helper::failed_exception("SmarteamProvider::SmarteamFromActiveObject QueryInterface error:", hr);
  }
